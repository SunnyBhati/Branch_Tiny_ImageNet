...
class BNFeatureHook():
    def __init__(self, module, save_path="./", training_momentum=0.99, name=None, gpu=0, flatness_weight=0,category_aware = 'global'):
        self.module = module
        ...
        # print(self.category_save_path_list)
        ...
        if category_aware == "global":
            for i, category_save_path in enumerate(self.category_save_path_list):
                if os.path.exists(category_save_path):
                    ...
                    # print(self.category_running_dd_var_list[i],len(self.category_running_dd_var_list[i]))
    ...
    def pre_hook_fn(self, module, input, output):
        # print(module)
        self.cntu+=1
        # print(self.cntu,'hjhjhj')
        nch = input[0].shape[1]
        bs = input[0].shape[0]
        input_0 = input[0]
        # print(nch,bs,input[0].shape,'life')
        # category-aware
        dd_mean = input_0.mean([2, 3])
        dd_var = input_0.view(bs, nch, -1).var(2, unbiased=False)
        # print(dd_mean.shape,dd_var.shape)
        for i in range(bs):
...
class ConvFeatureHook():
    ...
    def pre_hook_fn(self, module, input, output):
        ...
        bs = input[0].shape[0]
        input_0 = input[0]
        # print(nch,bs,input_0.shape)

        dd_mean = input_0.mean([0, 2, 3])  ##size 3
