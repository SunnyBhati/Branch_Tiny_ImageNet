...
def train_one_epoch(model, teacher_model, criterion, optimizer, data_loader, device, epoch, args, model_ema=None, scaler=None):
    ...
    for i, (image, target) in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        with torch.cuda.amp.autocast(enabled=scaler is not None):
            image, _, _, _ = utils.mix_aug(image, args)
    ...
    wandb.log({"relabel/train_loss": loss.item(), "relabel/train_acc": acc1.item()})
...
def load_data(traindir, valdir, args):
    ...
    train_transform = transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.RandomResizedCrop(64, scale=(0.5, 1), interpolation=InterpolationMode.BILINEAR),
...
def main(args):
    ...
    elif args.lr_scheduler == "smooth_cos":
        main_lr_scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer,
                             lambda step: 0.5 * (
                                     1. + math.cos(math.pi * step / (1.5 * args.epochs))) if step <= (1.5 * args.epochs) else 0,
                             last_epoch=-1)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        wandb.log({'relabel/lr': optimizer.param_groups[0]['lr'], 'relabel/acc': acc1})
    wandb.log({"relabel/best_acc": best_acc1})
...
if __name__ == "__main__":
    ...
    syn_dirname = os.path.basename(os.path.normpath(args.syn_data_path))
    wandb_id = syn_dirname.split('_')[0] if not args.run_id else args.run_id
    exp_name = syn_dirname.replace(f"{wandb_id}_", "") if not args.exp_name else args.exp_name
    args.output_dir = os.path.join(args.output_dir, f"{wandb_id}_{exp_name}")
    args.mix_type = 'cutmix'
    args.cutmix = 1.0
    args.mode = 'fkd_save'
    wandb.init(
        entity='priyamdey_iisc',
        project='dataset_distillation', 
        name=exp_name,
        id=wandb_id,
        config=args, 
        resume=True,
